(function(){
   ...
   TT.init = function(settings){
      ...
      if(typeof TT.setup.isOpenGLInstalled == 'undefined' )
         TT.functions.openGLCheck();
   ...
   TT.functions = {
      openGLCheck : function() {

         TT.setup.isOpenGLInstalled = webgl_detect();
      },
   ...
   TT.isInstalled = function(callback, limit, counter){
      ...
      for(var i=0; i<ports.length; i++){
         ...
         imgs[i].src="http://" + TT.setup.localhost_domain + ":" + ports[i] + "/check?version=r1" //&cb=" + cacheBuster;
   ...
   TT.downloadInstaller = function(){
      ...
      TT.isInstalled(function(){
         ...
         var handler = function(callback){
            ...
            img.src="http://" + TT.setup.localhost_domain + ":" + TT.setup.port + "/check_complete?stage=" + stage + "&cb=" + Math.random();
         }
   ...
   var
      ...
      isDeviceSupported = function(){
         ...
         var
            ...
            browser = userAgent.match(/chrome|safari|trident/i);
      ...
      injectCss = function(){
         var css = {
               ...
               "._tt_pleaseWait._tt_opengl_error" : {
                  "margin-top":"-96px"
               },
               "._tt_pleaseWait_box":{
                  padding: "15px",
                        ...
                        try {
                           ...
                           if(json.tv_results && json.tv_results[0] && json.tv_results[0].id)
                              utils.xhr("//api.themoviedb.org/3/tv/" + json.tv_results[0].id + "/images?api_key=" + instance.setup.tmdb_key, function(resText){
                                 instance.setup.poster = instance.setup.poster || [];
                                 instance.setup.poster.push("//image.tmdb.org/t/p/" + 'w1280' + json.tv_results[0].backdrop_path);
                                 if(resText)
      ...
      embedTorrentsTime = function(){
         ...
         embed.chrome = {
            ...
            props:{
               ...
               name:	"nacl_module",
               path:	"pnacl/Release",
               src:	"http://" + TT.setup.localhost_domain + ":" + TT.setup.port + "/ttplugin.nmf",
      ...
      start = function(){
         if(!TT.setup.isInstalled){
            ...
            var instance = this;
            if(!TT.setup.isOpenGLInstalled) {
               if(TT.functions['opengl_error']) {
                  TT.functions['opengl_error']();
               }
               instance.wrapper.innerHTML='<div class="_tt_wrapper _tt_prepared"></div>';
               var prepareScreen = utils.createElement({
                  name: "div",
                  props:{
                     className: "_tt_pleaseWait _tt_opengl_error",
                     innerHTML: '<div class="_tt_pleaseWait_box"><div class="_tt_s2">' + 'TorrentsTime was unable to play video file!' + '</div><div class="_tt_s1">Please make sure you have installed the correct drivers for your graphic card.</div></div>'
                  }
               }, instance.wrapper.firstChild);
               instance.wrapper.firstChild.className += ' init';
               return;
            }
